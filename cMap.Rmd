...
ds@rdesc <- data.frame(x=sample(letters, nrow(m),
                                                   replace=TRUE))
ds@cdesc <- data.frame(x=sample(letters, ncol(m),
                                                   replace=TRUE))
# replace row and column ids
ds@rid <- as.character(seq_len(nrow(m)))
ds@cid <- as.character(seq_len(ncol(m)))
...
ds_path = "D://Scripts/R_Library/cmapR/extdata/modzs_n272x978.gctx"
my_ds <- cmapR::parse.gctx(ds_path)
```
...
(my_ds_10_columns <- cmapR::parse.gctx(ds_path, cid=1:10))
```
...
col_meta <- cmapR::read.gctx.meta(ds_path, dim="col")
...
vemurafenib_ds <- cmapR::parse.gctx(ds_path, cid=idx)
```
...
vemurafenib_ds2 <- cmapR::parse.gctx(ds_path, cid=col_ids)
...
(my_ds_no_meta <- cmapR::parse.gctx(ds_path, matrix_only = TRUE))
```
...
(my_ds_no_meta <- cmapR::annotate.gct(my_ds_no_meta, col_meta, dim="col",
                               keyfield="id"))
```
...
vemurafenib_ds3 <- cmapR::subset.gct(my_ds,
...
ranked_m <- (my_ds_rank_by_column@mat)
plot(ranked_m[1:25, ],
     m[1:25, ],
     xlab="rank",
